...
export async function PATCH(
  ...
  try {
    ...
    const existingCase = await prisma.caseEng.findFirst({
      where: {
        slug: slug.trim(),
        NOT: {
          id: caseId,
    ...
    const updatedCase = await prisma.caseEng.update({
      where: {
        id: caseId,
      },
      data: {
    ...
    return NextResponse.json(
      {
        message: "Англ. кейс успішно оновлений",
        case: updatedCase,
      },
      { status: 200 },
    );
  } catch (error) {
    console.error("UPDATE_CASE_ENG_ERROR", error);
